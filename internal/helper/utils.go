...
func PopulateContainerTableUI(
	...
	if len(containers) == 0 {
		...
		table.SetCell(1, 0, tview.NewTableCell("No containers found in this state.").SetSelectable(false).SetTextColor(tcell.ColorYellow).SetAlign(tview.AlignCenter).SetExpansion(7))
		...
		for rowNum, container := range containers {
			...
			table.SetCell(rowIdx, 0, tview.NewTableCell(container.ID[:12]).SetAlign(tview.AlignLeft).SetReference(&containers[rowNum]).SetSelectedStyle(tcell.StyleDefault.Background(tcell.ColorDarkCyan)))
